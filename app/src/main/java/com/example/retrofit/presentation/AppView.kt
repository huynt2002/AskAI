...
import com.example.retrofit.domain.FakeRepository

@Composable
fun AppView(viewModel: ViewModel = hiltViewModel()) {
    val uiState = viewModel.uiState.collectAsStateWithLifecycle().value
    val listState = rememberLazyListState()

    LaunchedEffect(uiState.list.size) { listState.animateScrollToItem(0) }
    Scaffold { padding ->
        Column(
            modifier =
            ...
            LazyColumn(
                ...
                items(items = uiState.list.reversed(), key = { it.id }) { message ->
...
@Composable
private fun Preview() {
    val viewModel = ViewModel(aiRepository = FakeRepository())
    AppView(viewModel)
}
